...
async function main() {
  ...
  //
  await create(umi, { asset, name: NAME, uri: URI,
    plugins: [
        {
  ...
  const pubKey = asset.publicKey.toString();
  console.log(`pubKey: ${pubKey}`);
  console.log(`explorer link here: ${explorerAddress(pubKey)}`);
  // ── YOUR CODE ENDS HERE ──────────────────────────────────────────────
}
